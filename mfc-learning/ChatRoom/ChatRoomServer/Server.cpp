...
CChatRoomServerDlg* pDialog = NULL;
...
void printError(LPWSTR msg, int errCode)
{
	...
	pDialog->MessageBox(buffer, L"ERROR", MB_ICONERROR);
	LocalFree(formatted);
}
...
DWORD WINAPI clientHandler(LPVOID lpParameter) {
 ...
 DWORD WINAPI startServer(LPVOID lpParameter) {
	...
	pDialog = (CChatRoomServerDlg*)lpParameter;
	...
	pDialog->GetDlgItemText(IDC_EDIT_ServerIP, hostname);
	pDialog->GetDlgItemText(IDC_EDIT_ServerPort, port);
